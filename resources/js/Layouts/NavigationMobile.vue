    ...
    <aside v-show="$page.props.showingMobileMenu" class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-white md:hidden">
      <div class="py-4 text-gray-500 dark:text-gray-400">
        ...
        <ul class="mt-6">
          ...
          <li class="relative px-6 py-3">
            <BreezeNavLink :href="route('profile.my_subjects')" :active="route().current('profile.my_subjects')">
              <template #icon>
            ...
            <li class="relative px-6 py-3">
                <BreezeNavLink :href="route('moderator.student.index')" :active="route().current('moderator.student.index')">
                    <template #icon>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                             xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
